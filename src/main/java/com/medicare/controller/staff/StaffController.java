...
public class StaffController {
    ...
    //Lấy danh sách appointment đang pending
    @GetMapping("/appointments/pending")
    public ResponseEntity<List<AppointmentResponseDTO>> getPendingAppointments() {
        return ResponseEntity.ok(appointmentService.getPendingAppointmentsForStaff());
    }
    ...
    public ResponseEntity<String> updateAppointmentStatus(
        ...
        appointmentService.staffupdateAppointmentStatus(id, status);
    ...
    //Staff xử lý thanh toán tại quầy
    @PutMapping("/payments/{appointmentId}/pay-cash")
    public ResponseEntity<String> confirmCashPayment(@PathVariable UUID appointmentId) {
        paymentService.confirmCashPayment(appointmentId);
        return ResponseEntity.ok("Đã xác nhận thu tiền mặt thành công.");
    }
}
