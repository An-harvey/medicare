...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public ScheduleResponseDTO createSchedule(ScheduleCreateRequestDTO dto) {
        ...
        // Kiểm tra xem bác sĩ này đã có lịch làm việc tại ngày và khung giờ này chưa
        boolean isScheduleConflict = scheduleRepository.existsByDoctorProfile_DoctorIdAndWorkDateAndTimeSlot_Id(
                dto.getDoctorId(),
                dto.getWorkDate(),
                dto.getTimeSlotId()
        );
        ...
        if (isScheduleConflict) {
            throw new CustomException("Bác sĩ đã có lịch làm việc trong khung giờ này vào ngày được chọn. Vui lòng kiểm tra lại.", HttpStatus.BAD_REQUEST);
        }
    ...
    public void deleteSchedule(UUID scheduleId) {
        ...
        //  Kiểm tra xem có bất kỳ cuộc hẹn nào (kể cả đã CANCELLED) trỏ tới lịch này không
        boolean hasAppointments = appointmentRepository.existsBySchedule_Id(scheduleId);

        if (hasAppointments) {
            throw new CustomException(
                    "Không thể xóa lịch làm việc này vì đã lưu vết dữ liệu cuộc hẹn (lịch sử đặt khám/hủy khám) của bệnh nhân trong hệ thống. Nếu muốn vô hiệu hóa, vui lòng cập nhật trạng thái lịch thành CANCELLED.",
