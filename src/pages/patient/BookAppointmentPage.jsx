...
export default function Booking() {
  ...
  /* ── PUBLIC: load available slots khi đổi ngày ── */
  useEffect(() => {
    if (!doctorId || !selectedDate) return;
    setLoadingSlots(true);
    setSlot(null);

    // GET /api/public/schedules/available?doctorId=&date=yyyy-MM-dd
    // ScheduleResponseDTO: { scheduleId, doctorId, doctorName, workDate, startTime, maxPatients, currentPatients, status }
    getAvailableSlots({ doctorId, date: selectedDate })
      .then((res) => {
        const list = Array.isArray(res) ? res : res?.data ?? [];
        const map = {};
        list.forEach(s => {
          // startTime là "HH:mm:ss" — parse để hiển thị
          const timeLabel = s.startTime
            ? s.startTime.substring(0, 5) // "HH:mm"
            : (s.label || 'N/A');
          // AVAILABLE nếu currentPatients < maxPatients hoặc status === 'AVAILABLE'
          const available = s.status === 'AVAILABLE'
            || (s.currentPatients !== undefined && s.maxPatients !== undefined
  ...
  const handleSubmit = async () => {
    ...
    setApiError('');
    try {
      // POST /api/patient/appointments → AppointmentResponseDTO
  ...
  if (!doctor) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-gray-500">
        ...
        <Link to="/doctors" className="text-blue-600 text-sm hover:underline">← Quay lại danh sách</Link>
      </div>
    );
  }
