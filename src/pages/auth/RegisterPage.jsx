...
export default function Register() {
  ...
  const [form, setForm]       = useState({ name: '', email: '', phone: '', cccd: '', password: '', confirm: '' });
  ...
  const validate = () => {
    ...
    if (!/^\d{12}$/.test(form.cccd)) return 'CCCD phải đúng 12 chữ số.';
  ...
  const handleSubmit = async (e) => {
    ...
    // BE trả String message (201), KHÔNG tự login → redirect về /login
    const result = await register({ ...form, cccd: form.cccd });
    setLoading(false);
    if (!result.ok) { setError(result.error); return; }
    // Sau đăng ký thành công → chuyển sang trang login với thông báo
    navigate('/login', { state: { successMsg: result.message } });
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input name="cccd" value={form.cccd} onChange={handleChange}
                  placeholder="001099012345" maxLength={12}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-gray-50" />
              </div>
